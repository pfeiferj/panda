...
bool steer_angle_cmd_checks(int desired_angle, bool steer_control_enabled, const SteeringLimits limits) {
  ...
  // }} PFEIFER - AOL
    ...
    int highest_desired_angle = angle_meas.values[MAX_SAMPLE_VALS-1] + ((angle_meas.values[MAX_SAMPLE_VALS-1] > 0) ? delta_angle_up : delta_angle_down);
    int lowest_desired_angle = angle_meas.values[MAX_SAMPLE_VALS-1] - ((angle_meas.values[MAX_SAMPLE_VALS-1] >= 0) ? delta_angle_down : delta_angle_up);
    ...
    if (limits.enforce_angle_error && ((vehicle_speed.values[0] / VEHICLE_SPEED_FACTOR) > limits.angle_error_min_speed)) {
      ...
      int highest_desired_angle_lower = angle_meas.values[MAX_SAMPLE_VALS-1] + ((angle_meas.values[MAX_SAMPLE_VALS-1] > 0) ? delta_angle_up_lower : delta_angle_down_lower);
      int lowest_desired_angle_lower = angle_meas.values[MAX_SAMPLE_VALS-1] - ((angle_meas.values[MAX_SAMPLE_VALS-1] >= 0) ? delta_angle_down_lower : delta_angle_up_lower);
